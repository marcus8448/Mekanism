...
public class RenderBioGenerator extends MekanismTileEntityRenderer<TileEntityBioGenerator> {
    ...
    public RenderBioGenerator(BlockEntityRenderDispatcher renderer) {
        super(renderer);
    }

    @Override
    protected void render(TileEntityBioGenerator tile, float partialTick, MatrixStack matrix, VertexConsumerProvider renderer, int light, int overlayLight, Profiler profiler) {
        if (!tile.bioFuelTank.isEmpty()) {
            ...
            MekanismRenderer.renderObject(getModel(tile.getDirection(), (int) (fluidScale * (stages - 1))), matrix,
                  renderer.getBuffer(TexturedRenderLayers.getEntityTranslucentCull()), MekanismRenderer.getColorARGB(fluid, fluidScale), MekanismRenderer.FULL_LIGHT, overlayLight);
            matrix.pop();
        }
    }
