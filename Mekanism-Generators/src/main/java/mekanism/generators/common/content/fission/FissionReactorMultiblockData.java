...
public class FissionReactorMultiblockData extends MultiblockData implements IValveHandler {
    ...
    public void onCreated(World world) {
        ...
        hotZone = new Box(getMinPos().getX() + 1, getMinPos().getY() + 1, getMinPos().getZ() + 1,
    ...
    public void readUpdateTag(CompoundTag tag) {
        ...
        if (tag.contains(NBTConstants.ASSEMBLIES)) {
            ListTag list = tag.getList(NBTConstants.ASSEMBLIES, NBT.TAG_COMPOUND);
    ...
    private void radiateEntities(World world) {
        if (MekanismConfig.general.radiationEnabled.get() && isBurning() && world.getRandom().nextInt() % 20 == 0) {
            List<LivingEntity> entitiesToRadiate = getWorld().getNonSpectatingEntities(LivingEntity.class, hotZone);
