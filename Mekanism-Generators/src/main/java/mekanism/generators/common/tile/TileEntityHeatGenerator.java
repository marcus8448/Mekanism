...
public class TileEntityHeatGenerator extends TileEntityGenerator {
    ...
    private FloatingLong getBoost() {
        ...
        FloatingLong boost = MekanismGeneratorsConfig.generators.heatGenerationLava.get().multiply(Arrays.stream(EnumUtils.DIRECTIONS)
              .filter(side -> world.getFluidState(pos.offset(side)).isIn(FluidTags.LAVA)).count());
        if (world.getDimensionRegistryKey() == DimensionType.THE_NETHER_REGISTRY_KEY) {
    ...
    public IHeatHandler getAdjacent(Direction side) {
        if (side == Direction.DOWN) {
            BlockEntity adj = MekanismUtils.getTileEntity(getWorld(), pos.down());
