...
public abstract class BlockMekanism extends Block {
    ...
    public BlockEntity createTileEntity(@Nonnull BlockState state, @Nonnull BlockView world) {
        if (this instanceof IHasTileEntity) {
            return ((IHasTileEntity<?>) this).getTileType().instantiate();
        }
        return null;
    }
    ...
    @Override
    @Deprecated
    public BlockState getStateForNeighborUpdate(BlockState state, @Nonnull Direction facing, @Nonnull BlockState facingState, @Nonnull WorldAccess world, @Nonnull BlockPos currentPos,
    ...
    public void onPlaced(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull BlockState state, LivingEntity placer, @Nonnull ItemStack stack) {
        ...
        if (tile.supportsRedstone()) {
            tile.redstone = world.isReceivingRedstonePower(pos);
        }
        ...
        if (item instanceof ISecurityItem && tile.hasSecurity()) {
            ...
            tile.getSecurity().setOwnerUUID(ownerUUID == null ? placer.getUuid() : ownerUUID);
            if (!world.isClient) {
    ...
    @Override
    @Deprecated
    public boolean hasComparatorOutput(@Nonnull BlockState blockState) {
        return Attribute.has(this, AttributeComparator.class);
    }

    @Override
    @Deprecated
    public int getComparatorOutput(@Nonnull BlockState blockState, @Nonnull World world, @Nonnull BlockPos pos) {
        if (hasComparatorOutput(blockState)) {
            BlockEntity tile = MekanismUtils.getTileEntity(world, pos);
                ...
                for (int x = 0; x < xCount; x++) {
                    for (int y = 0; y < yCount; y++) {
                        for (int z = 0; z < zCount; z++) {
                            ...
                            manager.addParticle(new BlockDustParticle((ClientWorld) world, pos.getX() + d7, pos.getY() + d8,
    ...
    protected ActionResult genericClientActivated(@Nonnull PlayerEntity player, @Nonnull Hand hand, @Nonnull BlockHitResult hit) {
        ItemStack stack = player.getStackInHand(hand);
        if (stack.getItem() instanceof BlockItem && new ItemPlacementContext(player, hand, stack, hit).canPlace() && !Attribute.has(this, AttributeGui.class)) {
