...
public class BlockEnergyCube extends BlockTileModel<TileEntityEnergyCube, Machine<TileEntityEnergyCube>> {
    ...
    static {
        ...
        for (int rotated = 0; rotated < 3; rotated++) {
            ...
            for (int top = 0; top < 2; top++) {
                VoxelShape withTop = top == 0 ? baseFrame : VoxelShapes.union(baseFrame, rotateVertical ? topRotated : topPanel);
                for (int bottom = 0; bottom < 2; bottom++) {
                    VoxelShape withBottom = bottom == 0 ? withTop : VoxelShapes.union(withTop, rotateVertical ? bottomRotated : bottomPanel);
                    for (int front = 0; front < 2; front++) {
                        VoxelShape withFront = front == 0 ? withBottom : VoxelShapes.union(withBottom, frontPanel);
                        for (int back = 0; back < 2; back++) {
                            VoxelShape withBack = back == 0 ? withFront : VoxelShapes.union(withFront, backPanel);
                            for (int left = 0; left < 2; left++) {
                                VoxelShape withLeft = left == 0 ? withBack : VoxelShapes.union(withBack, rotateHorizontal ? leftRotated : leftPanel);
                                for (int right = 0; right < 2; right++) {
                                    VoxelShape withRight = right == 0 ? withLeft : VoxelShapes.union(withLeft, rotateHorizontal ? rightRotated : rightPanel);
    ...
    public BlockEnergyCube(Machine<TileEntityEnergyCube> type) {
        ...
        super(type, Block.Properties.of(Material.METAL).strength(2F, 4F).requiresTool().dynamicBounds());
    }
