...
public class BlockBin extends BlockTile<TileEntityBin, BlockTypeTile<TileEntityBin>> {
    ...
    @Override
    @Deprecated
    public void onBlockBreakStart(@Nonnull BlockState state, @Nonnull World world, @Nonnull BlockPos pos, @Nonnull PlayerEntity player) {
        if (!world.isClient) {
            ...
            if (mop.getType() != Type.MISS && mop.getSide() == bin.getDirection()) {
