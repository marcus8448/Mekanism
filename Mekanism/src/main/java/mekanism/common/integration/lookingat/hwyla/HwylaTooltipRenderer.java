...
public class HwylaTooltipRenderer implements IComponentProvider, ITooltipRenderer {
    ...
    public void appendBody(List<Text> tooltip, IDataAccessor accessor, IPluginConfig config) {
        ...
        if (data.contains(NBTConstants.MEK_DATA, NBT.TAG_LIST)) {
            ...
            ListTag list = data.getList(NBTConstants.MEK_DATA, NBT.TAG_COMPOUND);
            if (!list.isEmpty()) {
    ...
    public void draw(CompoundTag data, ICommonAccessor accessor, int x, int y) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (elementData.contains(MekanismHwylaPlugin.TEXT, NBT.TAG_STRING)) {
                Text text = Text.Serializer.fromJson(elementData.getString(MekanismHwylaPlugin.TEXT));
                if (text != null) {
                    LookingAtElement.renderScaledText(MinecraftClient.getInstance(), matrix, currentX + 4, currentY + 3, 0xFFFFFF, 92, text);
                    currentY += 15;
                }
                continue;
