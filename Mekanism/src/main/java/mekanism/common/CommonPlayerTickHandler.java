...
public class CommonPlayerTickHandler {
    ...
    public void onLivingJump(LivingJumpEvent event) {
        if (event.getEntityLiving() instanceof PlayerEntity) {
            PlayerEntity player = (PlayerEntity) event.getEntityLiving();
            ModuleHydraulicPropulsionUnit module = Modules.load(player.getEquippedStack(EquipmentSlot.FEET), Modules.HYDRAULIC_PROPULSION_UNIT);
            if (module != null && module.isEnabled() && Mekanism.keyMap.has(player.getUuid(), KeySync.BOOST)) {
                ...
                if (module.getContainerEnergy().greaterOrEqual(usage)) {
                    ...
                    ModuleLocomotiveBoostingUnit boostModule = Modules.load(player.getEquippedStack(EquipmentSlot.LEGS), Modules.LOCOMOTIVE_BOOSTING_UNIT);
                    if (boostModule != null && boostModule.isEnabled() && boostModule.canFunction(player)) {
                        boost = (float) Math.sqrt(boost);
                    }
                    player.setVelocity(player.getVelocity().add(0, boost, 0));
    ...
    private IEnergyContainer getFallAbsorptionEnergyContainer(LivingEntity base) {
        ItemStack feetStack = base.getEquippedStack(EquipmentSlot.FEET);
