...
public abstract class TileEntityBasicLaser extends TileEntityMekanism {
    ...
    protected void onUpdateServer() {
        ...
        if (!firing.isZero()) {
            ...
            BlockHitResult result = world.rayTrace(new RayTraceContext(from, to, ShapeType.COLLIDER, FluidHandling.NONE, dummy));
            if (result.getType() != Type.MISS) {
                to = new Pos3D(result.getPos());
            }
            ...
            List<Entity> hitEntities = world.getNonSpectatingEntities(Entity.class, Pos3D.getAABB(from, to));
            if (hitEntities.isEmpty()) {
                ...
                hitEntities.sort(Comparator.comparing(entity -> entity.squaredDistanceTo(finalFrom)));
                ...
                for (Entity entity : hitEntities) {
                    ...
                    if (entity instanceof LivingEntity) {
                        ...
                        if (livingEntity.isBlocking() && livingEntity.getActiveItem().isShield(livingEntity)) {
                            float damageBlocked = damageShield(livingEntity, livingEntity.getActiveItem(), damage, 2);
                    ...
                    if (!entity.isFireImmune()) {
                        entity.setOnFireFor(value.intValue());
                    }
                    int lastHurtResistTime = entity.timeUntilRegen;
                    //Set the hurt resistance time to zero to ensure we get a chance to do damage
                    entity.timeUntilRegen = 0;
                    boolean damaged = entity.damage(MekanismDamageSource.LASER, damage);
                    ...
                    entity.timeUntilRegen = lastHurtResistTime;
                    if (damaged) {
                        if (entity instanceof LivingEntity) {
            ...
            if (remainingEnergy.isZero() || result.getType() == Type.MISS) {
                ...
                Optional<ILaserReceptor> capability = MekanismUtils.toOptional(CapabilityUtils.getCapability(MekanismUtils.getTileEntity(world, hitPos),
                      Capabilities.LASER_RECEPTOR_CAPABILITY, result.getSide()));
                if (capability.isPresent() && !capability.get().canLasersDig()) {
                    //Give the energy to the receptor
                    capability.get().receiveLaserEnergy(remainingEnergy, result.getSide());
                } else {
                        ...
                        if (diggingProgress.compareTo(MekanismConfig.general.laserEnergyNeededPerHardness.get().multiply(hardness)) >= 0) {
                            if (MekanismConfig.general.aestheticWorldDamage.get()) {
                                ...
                                if (!MinecraftForge.EVENT_BUS.post(event)) {
                                    ...
                                    hitState.onStateReplaced(world, hitPos, Blocks.AIR.getDefaultState(), false);
                                    world.removeBlock(hitPos, false);
                                    world.syncWorldEvent(WorldEvents.BREAK_BLOCK_EFFECTS, hitPos, Block.getRawIdFromState(hitState));
                                }
                            }
                            diggingProgress = FloatingLong.ZERO;
                        } else {
    ...
    private float damageShield(LivingEntity livingEntity, ItemStack activeStack, float damage, int absorptionRation) {
        ...
        if (effectiveDamage >= 1) {
            ...
            activeStack.damage(durabilityNeeded, livingEntity, entity -> {
            ...
            if (activeStack.isEmpty()) {
                if (hand == Hand.MAIN_HAND) {
                    ...
                    livingEntity.equipStack(EquipmentSlot.OFFHAND, ItemStack.EMPTY);
                }
                livingEntity.clearActiveItem();
                livingEntity.playSound(SoundEvents.ITEM_SHIELD_BREAK, 0.8F, 0.8F + 0.4F * world.random.nextFloat());
        ...
        if (livingEntity instanceof ServerPlayerEntity && damageBlocked > 0 && damageBlocked < 3.4028235E37F) {
            ((ServerPlayerEntity) livingEntity).increaseStat(Stats.DAMAGE_BLOCKED_BY_SHIELD, Math.round(damageBlocked * 10F));
        }
    ...
    private void sendLaserDataToPlayers(LaserParticleData data, Pos3D from) {
        if (!isRemote() && world instanceof ServerWorld) {
            ...
            for (ServerPlayerEntity player : serverWorld.getPlayers()) {
                serverWorld.spawnParticles(player, data, true, from.x, from.y, from.z, 1, 0, 0, 0, 0);
            }
        }
    }
    ...
    protected void handleBreakBlock(BlockState state, BlockPos hitPos) {
        Block.dropStacks(state, world, hitPos, MekanismUtils.getTileEntity(world, hitPos));
    }
