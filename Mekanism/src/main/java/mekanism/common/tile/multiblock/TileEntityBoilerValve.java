...
public class TileEntityBoilerValve extends TileEntityBoilerCasing {
    ...
    private BoilerValveMode getMode() {
        return getCachedState().get(AttributeStateBoilerValveMode.modeProperty);
    }
    ...
    public ActionResult onSneakRightClick(PlayerEntity player, Direction side) {
        if (!isRemote()) {
            ...
            world.setBlockState(pos, getCachedState().with(AttributeStateBoilerValveMode.modeProperty, mode));
