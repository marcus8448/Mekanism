...
public class TileEntityLogisticalSorter extends TileEntityMekanism implements ISpecialConfigData, ISustainedData, ITileFilterHolder<SorterFilter<?>>,
    ...
    protected void onUpdateServer() {
        ...
        if (MekanismUtils.canFunction(this) && delayTicks == 0) {
            BlockEntity back = MekanismUtils.getTileEntity(getWorld(), pos.offset(getOppositeDirection()));
            BlockEntity front = MekanismUtils.getTileEntity(getWorld(), pos.offset(getDirection()));
    ...
    public boolean hasConnectedInventory() {
        BlockEntity tile = MekanismUtils.getTileEntity(getWorld(), pos.offset(getOppositeDirection()));
