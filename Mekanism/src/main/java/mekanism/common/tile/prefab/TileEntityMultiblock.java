...
public abstract class TileEntityMultiblock<T extends MultiblockData> extends TileEntityMekanism implements IMultiblock<T>, IConfigurable {
    ...
    protected void onUpdateServer() {
        ...
        if (!getMultiblock().isFormed()) {
            playersUsing.forEach(PlayerEntity::closeHandledScreen);
    ...
    private void structureChanged() {
        ...
        for (Direction side : EnumUtils.DIRECTIONS) {
            ...
            if (!getMultiblock().isFormed() || (!getMultiblock().locations.contains(pos) && !getMultiblock().internalLocations.contains(pos))) {
                ...
                if (!world.isAir(pos) && (tile == null || tile.getClass() != getClass()) && !(tile instanceof IStructuralMultiblock || tile instanceof IMultiblock)) {
    ...
    @Override
    public ActionResult onActivate(PlayerEntity player, Hand hand, ItemStack stack) {
    ...
    public Box getRenderBoundingBox() {
        if (getMultiblock().isFormed() && isMaster && getMultiblock().getBounds() != null) {
            ...
            return new Box(getMultiblock().getMinPos(), getMultiblock().getMaxPos().add(1, 1, 1));
        }
    ...
    public void onNeighborChange(Block block, BlockPos neighborPos) {
        ...
        if (!isRemote() && getMultiblock().isPositionInsideBounds(getStructure(), neighborPos)) {
            ...
            if (!getMultiblock().innerNodes.contains(neighborPos) || world.isAir(neighborPos)) {
    ...
    public ActionResult onRightClick(PlayerEntity player, Direction side) {
        if (!isRemote() && !getMultiblock().isFormed()) {
            ...
            if (!result.isFormed() && result.getResultText() != null) {
                player.sendSystemMessage(result.getResultText(), Util.NIL_UUID);
