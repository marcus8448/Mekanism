...
public class TileEntityDigitalMiner extends TileEntityMekanism implements ISustainedData, IChunkLoader, IAdvancedBoundingBlock, ITileFilterHolder<MinerFilter<?>>,
    ...
    private void closeInvalidScreens() {
        if (getActive() && !playersUsing.isEmpty()) {
            for (PlayerEntity player : new ObjectOpenHashSet<>(playersUsing)) {
                if (player.currentScreenHandler instanceof DigitalMinerConfigContainer) {
    ...
    protected void onUpdateServer() {
                ...
                if (delay == 0) {
                    ...
                    for (Iterator<ChunkPos> it = oresToMine.keySet().iterator(); it.hasNext(); ) {
                        ...
                        while (!did) {
                            ...
                            if (!world.canSetBlock(pos) || world.isAir(pos)) {
                                set.clear(index);
        ...
        if (doEject && delayTicks == 0) {
            BlockEntity ejectInv = getEjectInv();
            BlockEntity ejectTile = getEjectTile();
    ...
    private TransitRequest getEjectItemMap(BlockEntity tile, Direction side) {
    ...
    private BlockEntity getEjectInv() {
    ...
    public void start() {
        ...
        if (searcher.state == State.IDLE) {
            ...
            searcher.setChunkCache(new ChunkCache(getWorld(), startingPos, startingPos.add(getDiameter(), getMaxY() - getMinY() + 1, getDiameter())));
            searcher.start();
        }
        running = true;
        markDirty(false);
    ...
    public Box getRenderBoundingBox() {
        ...
        return new Box(pos.add(-1, 0, -1), pos.add(2, 2, 2));
    }
                ...
                for (int y = 0; y <= +1; y++) {
                    for (int z = -1; z <= +1; z++) {
                        if (x != 0 || y != 0 || z != 0) {
                            ...
                            world.updateNeighborsAlways(boundingPos, getBlockType());
                        }
                    }
                }
            }
    ...
    private BlockEntity getEjectTile() {
    ...
    public void setConfigurationData(CompoundTag nbtTags) {
        ...
        if (nbtTags.contains(NBTConstants.FILTERS, NBT.TAG_LIST)) {
            ListTag tagList = nbtTags.getList(NBTConstants.FILTERS, NBT.TAG_COMPOUND);
    ...
    public boolean isOffsetCapabilityDisabled(@Nonnull Capability<?> capability, Direction side, @Nonnull Vec3i offset) {
        if (capability == CapabilityItemHandler.ITEM_HANDLER_CAPABILITY) {
            ...
            if (offset.equals(new Vec3i(back.getOffsetX(), 1, back.getOffsetZ()))) {
            ...
            if (offset.equals(new Vec3i(left.getOffsetX(), 0, left.getOffsetZ()))) {
                //Disable if left power port but wrong side of the port
                return side != left;
            } else if (offset.equals(new Vec3i(right.getOffsetX(), 0, right.getOffsetZ()))) {
