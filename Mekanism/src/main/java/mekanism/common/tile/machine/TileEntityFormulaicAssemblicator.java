...
public class TileEntityFormulaicAssemblicator extends TileEntityMekanism implements ISideConfiguration, IConfigCardAccess, IHasMode {
    ...
    private Optional<CraftingRecipe> cachedRecipe = Optional.empty();
    private DefaultedList<ItemStack> lastRemainingItems = EMPTY_LIST;
    ...
    private void loadFormula() {
        ...
        DefaultedList<ItemStack> formulaInventory = formulaItem.getInventory(formulaStack);
        ...
        if (world != null && !isRemote()) {
            if (formula == null || !formula.isValidFormula()) {
                ...
                for (int i = 0; i < craftingGridSlots.size(); i++) {
                    dummyInv.setStack(i, StackUtils.size(craftingGridSlots.get(i).getStack(), 1));
                }
                ...
                if (!cachedRecipe.isPresent() || !cachedRecipe.get().matches(dummyInv, world)) {
                    ...
                    cachedRecipe = world.getRecipeManager().getFirstMatch(RecipeType.CRAFTING, dummyInv, world);
                }
                if (cachedRecipe.isPresent()) {
                    lastOutputStack = cachedRecipe.get().craft(dummyInv);
                    lastRemainingItems = cachedRecipe.get().getRemainingStacks(dummyInv);
                } else {
                ...
                if (isRecipe) {
                    lastOutputStack = formula.recipe.craft(dummyInv);
                    lastRemainingItems = formula.recipe.getRemainingStacks(dummyInv);
                } else {
    ...
    private static ItemStack getStackFromMap(Map<HashedItem, Integer> map, HashedItem item) {
        ...
        ItemStack ret = item.createStack(Math.min(item.getStack().getMaxCount(), stored));
        ...
        container.track(SyncableBoolean.create(() -> formula != null, hasFormula -> {
            if (hasFormula) {
                if (formula == null && isRemote()) {
                    ...
                    formula = new RecipeFormula(getWorld(), DefaultedList.ofSize(9, ItemStack.EMPTY));
                }
            } else {
                formula = null;
            }
