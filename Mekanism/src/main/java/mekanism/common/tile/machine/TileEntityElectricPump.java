...
public class TileEntityElectricPump extends TileEntityMekanism implements IConfigurable {
    ...
    private boolean suck(BlockPos pos, boolean hasFilter, boolean addRecurring) {
        ...
        if (!fluidState.isEmpty() && fluidState.isStill()) {
            ...
            } else if (block instanceof FluidDrainable && validFluid(fluidStack, false)) {
    ...
    public CompoundTag toTag(@Nonnull CompoundTag nbtTags) {
        ...
        for (BlockPos nodePos : recurringNodes) {
            recurringList.add(NbtHelper.fromBlockPos(nodePos));
        }
    ...
    public void fromTag(@Nonnull BlockState state, @Nonnull CompoundTag nbtTags) {
        ...
        if (nbtTags.contains(NBTConstants.RECURRING_NODES, NBT.TAG_LIST)) {
            ListTag tagList = nbtTags.getList(NBTConstants.RECURRING_NODES, NBT.TAG_COMPOUND);
            for (int i = 0; i < tagList.size(); i++) {
                recurringNodes.add(NbtHelper.toBlockPos(tagList.getCompound(i)));
            }
        }
    }
    ...
    public ActionResult onSneakRightClick(PlayerEntity player, Direction side) {
        reset();
        player.sendSystemMessage(MekanismLang.LOG_FORMAT.translateColored(EnumColor.DARK_BLUE, MekanismLang.MEKANISM, EnumColor.GRAY, MekanismLang.PUMP_RESET), Util.NIL_UUID);
    ...
    @Override
    public List<Text> getInfo(Upgrade upgrade) {
