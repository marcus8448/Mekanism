...
public class SPSMultiblockData extends MultiblockData implements IValveHandler {
    ...
    public void onCreated(World world) {
        ...
        deathZone = new Box(getMinPos().getX() + 2, getMinPos().getY() + 2, getMinPos().getZ() + 2,
    ...
    private void kill(World world) {
        if (!lastReceivedEnergy.isZero() && couldOperate && world.getRandom().nextInt() % 20 == 0) {
            List<Entity> entitiesToDie = getWorld().getNonSpectatingEntities(Entity.class, deathZone);
            for (Entity entity : entitiesToDie) {
                entity.damage(DamageSource.MAGIC, lastReceivedEnergy.floatValue() / 1_000F);
            }
        }
    }
    ...
    public static class SyncableCoilData {
        ...
        public void write(CompoundTag tags) {
            ...
            for (CoilData data : coilMap.values()) {
                ...
                tag.put(NBTConstants.POSITION, NbtHelper.fromBlockPos(data.coilPos));
        ...
        public void read(CompoundTag tags) {
            coilMap.clear();
            ListTag list = tags.getList(NBTConstants.COILS, NBT.TAG_COMPOUND);
            for (int i = 0; i < list.size(); i++) {
                CompoundTag tag = list.getCompound(i);
                BlockPos pos = NbtHelper.toBlockPos(tag.getCompound(NBTConstants.POSITION));
                Direction side = Direction.byId(tag.getInt(NBTConstants.SIDE));
