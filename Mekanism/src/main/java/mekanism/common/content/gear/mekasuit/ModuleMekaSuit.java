...
public abstract class ModuleMekaSuit extends Module {
    ...
    public static class ModuleElectrolyticBreathingUnit extends ModuleMekaSuit {
        ...
        public void tickServer(PlayerEntity player) {
            ...
            if (fluidstate.isIn(FluidTags.WATER) && maskHeight <= headPos.getY() + fluidstate.getHeight(player.getEntityWorld(), headPos)) {
                //If the position the bottom of the mask is in is water set the production rate to our max rate
                productionRate = getMaxRate();
            } else if (player.isBeingRainedOn()) {
    ...
    public static class ModuleSolarRechargingUnit extends ModuleMekaSuit {
        ...
        public void tickServer(PlayerEntity player) {
            ...
            if (energyContainer != null && !energyContainer.getNeeded().isZero() && player.world.isDay() &&
                player.world.isSkyVisible(new BlockPos(player.getPos())) && !player.world.isRaining() && player.world.getDimension().hasSkyLight()) {
    ...
    public static class ModuleDosimeterUnit extends ModuleMekaSuit {
        ...
        public void addHUDElements(List<HUDElement> list) {
            ...
            Optional<IRadiationEntity> capability = MekanismUtils.toOptional(CapabilityUtils.getCapability(MinecraftClient.getInstance().player,
