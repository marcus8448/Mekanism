...
public class SearchQueryParser {
    ...
    public enum QueryType {
        ...
        NAME('~', (key, stack) -> stack.getName().getString().toLowerCase(Locale.ROOT).contains(key.toLowerCase(Locale.ROOT))),
        MOD_ID('@', (key, stack) -> stack.getItem().getRegistryName().getNamespace().toLowerCase(Locale.ROOT).contains(key.toLowerCase(Locale.ROOT))),
        TOOLTIP('$', (key, stack) -> stack.getTooltip(null, TooltipContext.Default.NORMAL).stream().map(t -> t.getString().toLowerCase(Locale.ROOT))
