...
public final class TransporterPathfinder {
    ...
    private static List<Destination> getPaths(LogisticalTransporterBase start, TransporterStack stack, TransitRequest request, int min) {
        ...
        Long2ObjectMap<Chunk> chunkMap = new Long2ObjectOpenHashMap<>();
    ...
    private static boolean checkPath(World world, List<BlockPos> path, TransporterStack stack, Long2ObjectMap<Chunk> chunkMap) {
        for (int i = path.size() - 1; i > 0; i--) {
            BlockEntity tile = MekanismUtils.getTileEntity(world, chunkMap, path.get(i));
    ...
    private static Destination getPath(AcceptorData data, LogisticalTransporterBase start, TransporterStack stack, int min, Long2ObjectMap<Chunk> chunkMap) {
        ...
        if (response.getSendingAmount() >= min) {
            ...
            Pathfinder p = new Pathfinder(new DestChecker() {
                @Override
                public boolean isValid(TransporterStack stack, Direction dir, BlockEntity tile) {
    ...
    public static class IdlePath {
        ...
        public Destination find(Long2ObjectMap<Chunk> chunkMap) {
            ArrayList<BlockPos> ret = new ArrayList<>();
            ret.add(start);
            BlockEntity startTile = MekanismUtils.getTileEntity(world, chunkMap, start);
        ...
        private void loopSide(Long2ObjectMap<Chunk> chunkMap, List<BlockPos> list, Direction side, BlockEntity startTile) {
            BlockEntity lastTile = startTile;
            BlockPos pos = start.offset(side);
            BlockEntity tile = MekanismUtils.getTileEntity(world, chunkMap, pos);
        ...
        private Direction findSide(Long2ObjectMap<Chunk> chunkMap) {
            BlockEntity startTile = MekanismUtils.getTileEntity(world, chunkMap, start);
            if (transportStack.idleDir == null) {
    ...
    public static class Pathfinder {
        ...
        public Pathfinder(DestChecker checker, World world, BlockPos finishObj, BlockPos startObj, TransporterStack stack, Long2ObjectMap<Chunk> chunkMap) {
            destChecker = checker;
            this.world = world;
        ...
        public boolean find(Long2ObjectMap<Chunk> chunkMap, BlockPos start) {
            openSet.add(start);
            gScore.put(start, 0D);
            ...
            for (Direction direction : EnumUtils.DIRECTIONS) {
                ...
                BlockEntity neighborTile = MekanismUtils.getTileEntity(world, chunkMap, neighbor);
            ...
            while (!openSet.isEmpty()) {
                ...
                BlockEntity currentNodeTile = MekanismUtils.getTileEntity(world, chunkMap, currentNode);
                double currentScore = gScore.getDouble(currentNode);
                for (Direction direction : EnumUtils.DIRECTIONS) {
                    BlockPos neighbor = currentNode.offset(direction);
                    BlockEntity neighborEntity = MekanismUtils.getTileEntity(world, chunkMap, neighbor);
         ...
         */
        private boolean isValidDestination(BlockPos start, BlockEntity startTile, Direction direction, BlockPos neighbor, BlockEntity neighborTile) {
