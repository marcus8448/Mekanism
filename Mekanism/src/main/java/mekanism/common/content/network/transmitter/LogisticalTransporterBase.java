...
public abstract class LogisticalTransporterBase extends Transmitter<IItemHandler, InventoryNetwork, LogisticalTransporterBase> {
                    ...
                    if (stack.hasPath()) {
                        ...
                        if (next != null) {
                            if (!stack.isFinal(this)) {
                                ...
                                BlockEntity tile = MekanismUtils.getTileEntity(getTileWorld(), next);
                                if (tile != null) {
    ...
    public void handleUpdateTag(@Nonnull CompoundTag tag) {
        ...
        if (tag.contains(NBTConstants.ITEMS, NBT.TAG_LIST)) {
            ListTag tagList = tag.getList(NBTConstants.ITEMS, NBT.TAG_COMPOUND);
    ...
    public void readStacksFromNBT(ListTag tagList) {
        for (int i = 0; i < tagList.size(); i++) {
    ...
    public void writeToNBT(CompoundTag nbtTags) {
        ListTag stacks = writeStackToNBT();
        if (!stacks.isEmpty()) {
            nbtTags.put(NBTConstants.ITEMS, stacks);
        }
    }

    public ListTag writeStackToNBT() {
        ListTag stacks = new ListTag();
        for (TransporterStack stack : getTransit()) {
            CompoundTag tagCompound = new CompoundTag();
