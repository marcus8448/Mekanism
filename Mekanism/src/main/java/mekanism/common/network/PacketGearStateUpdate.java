...
public class PacketGearStateUpdate {
    ...
    public static void encode(PacketGearStateUpdate pkt, PacketByteBuf buf) {
        buf.writeEnumConstant(pkt.gearType);
        buf.writeUuid(pkt.uuid);
        buf.writeBoolean(pkt.state);
    }

    public static PacketGearStateUpdate decode(PacketByteBuf buf) {
        return new PacketGearStateUpdate(buf.readEnumConstant(GearType.class), buf.readUuid(), buf.readBoolean());
    }
