    ...
    public enum GuiInteraction {//TODO: Cleanup this enum/the elements in it as it is rather disorganized order wise currently
        CONTAINER_STOP_TRACKING((tile, player, extra) -> {
            if (player.currentScreenHandler instanceof MekanismContainer) {
                ((MekanismContainer) player.currentScreenHandler).stopTracking(extra);
            }
        }),
        CONTAINER_TRACK_EJECTOR((tile, player, extra) -> {
            if (player.currentScreenHandler instanceof MekanismContainer && tile instanceof ISideConfiguration) {
                ((MekanismContainer) player.currentScreenHandler).startTracking(extra, ((ISideConfiguration) tile).getEjector());
            }
        }),
        CONTAINER_TRACK_SIDE_CONFIG((tile, player, extra) -> {
            if (player.currentScreenHandler instanceof MekanismContainer && tile instanceof ISideConfiguration) {
                ((MekanismContainer) player.currentScreenHandler).startTracking(extra, ((ISideConfiguration) tile).getConfig());
            }
        }),
        ...
        REMOVE_UPGRADE((tile, player, extra) -> {
            if (tile.supportsUpgrades()) {
                ...
                if (componentUpgrade.getUpgrades(upgradeType) > 0 && player.inventory.insertStack(UpgradeUtils.getStack(upgradeType))) {
