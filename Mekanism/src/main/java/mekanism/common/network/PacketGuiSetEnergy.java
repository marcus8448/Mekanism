...
public class PacketGuiSetEnergy {
    ...
    public static void encode(PacketGuiSetEnergy pkt, PacketByteBuf buf) {
        buf.writeEnumConstant(pkt.interaction);
        buf.writeBlockPos(pkt.tilePosition);
        pkt.value.writeToBuffer(buf);
    }

    public static PacketGuiSetEnergy decode(PacketByteBuf buf) {
        return new PacketGuiSetEnergy(buf.readEnumConstant(GuiEnergyValue.class), buf.readBlockPos(), FloatingLong.readFromBuffer(buf));
    }
