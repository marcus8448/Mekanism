...
public class PacketUpdateContainerBatch {
    ...
    public static void handle(PacketUpdateContainerBatch message, Supplier<Context> context) {
        ...
        ctx.enqueueWork(() -> {
            ...
            if (player.currentScreenHandler instanceof MekanismContainer && player.currentScreenHandler.syncId == message.windowId) {
                ...
                message.data.forEach(data -> data.handleWindowProperty((MekanismContainer) player.currentScreenHandler));
            }
        });
        ctx.setPacketHandled(true);
    }

    public static void encode(PacketUpdateContainerBatch pkt, PacketByteBuf buffer) {
    ...
    public static PacketUpdateContainerBatch decode(PacketByteBuf buffer) {
