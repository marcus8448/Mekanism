...
public abstract class ListPropertyData<TYPE> extends PropertyData {
    ...
    public static <TYPE> ListPropertyData<TYPE> readList(short property, PacketByteBuf buffer) {
        ListType listType = buffer.readEnumConstant(ListType.class);
    ...
    public void writeToPacket(PacketByteBuf buffer) {
        ...
        buffer.writeEnumConstant(listType);
        buffer.writeVarInt(values.size());
        writeListElements(buffer);
    }

    protected abstract void writeListElements(PacketByteBuf buffer);
}