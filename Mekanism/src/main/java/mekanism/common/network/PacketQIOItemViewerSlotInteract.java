...
public class PacketQIOItemViewerSlotInteract {
    ...
    public static void handle(PacketQIOItemViewerSlotInteract message, Supplier<Context> context) {
        ...
        context.get().enqueueWork(() -> {
            if (player.currentScreenHandler instanceof QIOItemViewerContainer) {
                ...
                ItemStack curStack = player.inventory.getCursorStack();
                if (freq != null) {
                    if (message.type == Type.TAKE) {
                        ItemStack ret = freq.removeByType(message.itemType, message.count);
                        if (curStack.isEmpty()) {
                            player.inventory.setCursorStack(ret);
                        } else if (InventoryUtils.areItemsStackable(ret, curStack)) {
                            curStack.increment(ret.getCount());
                        }
                        ((ServerPlayerEntity) player).networkHandler.sendPacket(new ScreenHandlerSlotUpdateS2CPacket(-1, -1, player.inventory.getCursorStack()));
                    } else if (message.type == Type.SHIFT_TAKE) {
                        ItemStack ret = freq.removeByType(message.itemType, message.itemType.getStack().getMaxCount());
    ...
    public static void encode(PacketQIOItemViewerSlotInteract pkt, PacketByteBuf buf) {
    ...
    public static PacketQIOItemViewerSlotInteract decode(PacketByteBuf buf) {
