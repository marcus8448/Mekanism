...
public class PacketTransporterUpdate {
    ...
    public static void encode(PacketTransporterUpdate pkt, PacketByteBuf buf) {
        ...
        if (pkt.isDiversion) {
            for (int i = 0; i < pkt.modes.length; i++) {
                buf.writeEnumConstant(pkt.modes[i]);
            }
        }
    }

    public static PacketTransporterUpdate decode(PacketByteBuf buf) {
        ...
        if (packet.isDiversion) {
            ...
            for (int i = 0; i < packet.modes.length; i++) {
                packet.modes[i] = buf.readEnumConstant(DiversionControl.class);
            }
        }
        return packet;
    }
