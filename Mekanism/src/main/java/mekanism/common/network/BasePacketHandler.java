...
public abstract class BasePacketHandler {
    ...
    protected static SimpleChannel createChannel(Identifier name) {
    ...
    public static Vec3d readVector3d(PacketByteBuf buffer) {
        return new Vec3d(buffer.readDouble(), buffer.readDouble(), buffer.readDouble());
    }

    public static void writeVector3d(PacketByteBuf buffer, Vec3d vector) {
    ...
    protected <MSG> void registerClientToServer(Class<MSG> type, BiConsumer<MSG, PacketByteBuf> encoder, Function<PacketByteBuf, MSG> decoder,
    ...
    protected <MSG> void registerServerToClient(Class<MSG> type, BiConsumer<MSG, PacketByteBuf> encoder, Function<PacketByteBuf, MSG> decoder,
          BiConsumer<MSG, Supplier<Context>> consumer) {
    ...
    public <MSG> void sendTo(MSG message, ServerPlayerEntity player) {
        getChannel().sendTo(message, player.networkHandler.getConnection(), NetworkDirection.PLAY_TO_CLIENT);
    }
    ...
    public <MSG> void sendToAllTracking(MSG message, World world, BlockPos pos) {
        if (world instanceof ServerWorld) {
            ...
            ((ServerWorld) world).getChunkManager().threadedAnvilChunkStorage.getPlayersWatchingChunk(new ChunkPos(pos), false).forEach(p -> sendTo(message, p));
        } else {
        ...
        try {
            ...
            if (server != null) {
                ...
                for (ServerPlayerEntity player : playerList.getPlayerList()) {
                    if (range.dimension == player.getSpawnPointDimension()) {
