...
public class ItemBlockMachine extends ItemBlockTooltip<BlockTile<?, ?>> implements IItemSustainedInventory, ISecurityItem {
    ...
    public ItemBlockMachine(BlockTile<?, ?> block, Supplier<Callable<BuiltinModelItemRenderer>> renderer) {
        super(block, true, ItemDeferredRegister.getMekBaseProperties().maxCount(1).setISTER(renderer));
    }
