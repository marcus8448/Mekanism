...
public class ItemBlockFluidTank extends ItemBlockTooltip<BlockFluidTank> implements IItemSustainedInventory, ISecurityItem, IModeItem {
    ...
    public ItemBlockFluidTank(BlockFluidTank block) {
        super(block, true, ItemDeferredRegister.getMekBaseProperties().maxCount(1).setISTER(ISTERProvider::fluidTank));
    }
    ...
    public TypedActionResult<ItemStack> use(@Nonnull World world, PlayerEntity player, @Nonnull Hand hand) {
        ...
        if (getBucketMode(stack)) {
            if (SecurityUtils.canAccess(player, stack)) {
                BlockHitResult result = rayTrace(world, player, !player.isSneaking() ? FluidHandling.SOURCE_ONLY : FluidHandling.NONE);
                ...
                if (result.getType() == Type.BLOCK) {
                    ...
                    if (!player.isSneaking()) {
                        if (!player.canPlaceOn(pos, result.getSide(), stack)) {
                            return new TypedActionResult<>(ActionResult.FAIL, stack);
                        }
                        ...
                        if (!fluidState.isEmpty() && fluidState.isStill()) {
                            ...
                            if (block instanceof IFluidBlock) {
                            ...
                            } else if (block instanceof FluidDrainable && validFluid(fluidTank, fluidStack)) {
                                ...
                                fluid = ((FluidDrainable) block).tryDrainFluid(world, pos, blockState);
                        ...
                        if (fluidTank.extract(FluidAttributes.BUCKET_VOLUME, Action.SIMULATE, AutomationType.MANUAL).getAmount() < FluidAttributes.BUCKET_VOLUME
                            || !player.canPlaceOn(pos.offset(result.getSide()), result.getSide(), stack)) {
                            return new TypedActionResult<>(ActionResult.FAIL, stack);
                        }
                        if (MekanismUtils.tryPlaceContainedLiquid(player, world, pos, fluidHandlerItem.getFluidInTank(0), result.getSide())) {
