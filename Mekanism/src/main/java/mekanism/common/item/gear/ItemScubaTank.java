...
public class ItemScubaTank extends ItemGasArmor implements IItemHUDProvider, IModeItem {
    ...
    public ItemScubaTank(Settings properties) {
        super(SCUBA_TANK_MATERIAL, EquipmentSlot.CHEST, properties.setISTER(ISTERProvider::scubaTank));
    }
        ...
        if (Math.abs(shift) % 2 == 1) {
            ...
            if (displayChangeMessage) {
                player.sendSystemMessage(MekanismLang.LOG_FORMAT.translateColored(EnumColor.DARK_BLUE, MekanismLang.MEKANISM,
                      MekanismLang.FLOWING.translateColored(EnumColor.GRAY, OnOff.of(newState, true))), Util.NIL_UUID);
            }
        }
    }
