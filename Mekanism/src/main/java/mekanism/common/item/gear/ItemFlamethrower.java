...
public class ItemFlamethrower extends Item implements IItemHUDProvider, IModeItem, IGasItem {
    ...
    public ItemFlamethrower(Settings properties) {
        super(properties.maxCount(1).rarity(Rarity.RARE).setNoRepair().setISTER(ISTERProvider::flamethrower));
    }
    ...
    public boolean shouldCauseReequipAnimation(ItemStack oldStack, ItemStack newStack, boolean slotChanged) {
        return slotChanged || !ItemStack.areItemsEqualIgnoreDamage(oldStack, newStack);
    }
        ...
        if (mode != newMode) {
            ...
            if (displayChangeMessage) {
                player.sendSystemMessage(MekanismLang.LOG_FORMAT.translateColored(EnumColor.DARK_BLUE, MekanismLang.MEKANISM,
                      MekanismLang.FLAMETHROWER_MODE_CHANGE.translateColored(EnumColor.GRAY, newMode)), Util.NIL_UUID);
            }
        }
    }
