...
public class ItemJetpack extends ItemGasArmor implements IItemHUDProvider, IModeItem {
    ...
    public ItemJetpack(Settings properties) {
        this(JETPACK_MATERIAL, properties.setISTER(ISTERProvider::jetpack));
    }

    public ItemJetpack(ArmorMaterial material, Settings properties) {
        ...
        if (mode != newMode) {
            ...
            if (displayChangeMessage) {
                player.sendSystemMessage(MekanismLang.LOG_FORMAT.translateColored(EnumColor.DARK_BLUE, MekanismLang.MEKANISM,
                      MekanismLang.JETPACK_MODE_CHANGE.translateColored(EnumColor.GRAY, newMode)), Util.NIL_UUID);
            }
        }
    }
    ...
    public enum JetpackMode implements IIncrementalEnum<JetpackMode>, IHasTextComponent {
        ...
        JetpackMode(ILangEntry langEntry, EnumColor color, Identifier hudIcon) {
