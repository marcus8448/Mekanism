...
public class InsertableSlot extends Slot implements IInsertableSlot {
    ...
    public InsertableSlot(Inventory inventory, int index, int x, int y) {
        super(inventory, index, x, y);
    }

    @Override
    public int getMaxStackAmount(ItemStack stack) {
        return Math.min(getMaxStackAmount(), stack.getMaxCount());
    }
    ...
    public ItemStack insertItem(@Nonnull ItemStack stack, Action action) {
        ...
        int needed = getMaxStackAmount(stack) - current.getCount();
        if (needed <= 0) {
        ...
        if (current.isEmpty() || ItemHandlerHelper.canItemStacksStack(current, stack)) {
            ...
            if (action.execute()) {
                ...
                setStack(StackUtils.size(stack, current.getCount() + toAdd));
            }
