...
public class PersonalChestItemContainer extends MekanismItemContainer {
    ...
    public PersonalChestItemContainer(int id, PlayerInventory inv, PacketByteBuf buf) {
        this(id, inv, buf.readEnumConstant(Hand.class), getStackFromBuffer(buf, ItemBlockPersonalChest.class));
    }
    ...
    @Nonnull
    @Override
    public ItemStack onSlotClick(int slotId, int dragType, @Nonnull SlotActionType clickType, @Nonnull PlayerEntity player) {
        //TODO: Re-evaluate
        ...
        if (hotbarSlotId >= 0 && hotbarSlotId < 9 && player.inventory.selectedSlot == hotbarSlotId) {
            ItemStack itemStack = player.inventory.getStack(hotbarSlotId);
            if (!itemStack.isEmpty() && itemStack.getItem() instanceof ItemBlockPersonalChest) {
                return ItemStack.EMPTY;
            }
        }
        return super.onSlotClick(slotId, dragType, clickType, player);
    }
}