...
public class MekanismTileContainer<TILE extends TileEntityMekanism> extends MekanismContainer {
    ...
    public static <TILE extends BlockEntity> TILE getTileFromBuf(PacketByteBuf buf, Class<TILE> type) {
        if (buf == null) {
            return null;
        }
        return DistExecutor.unsafeCallWhenOn(Dist.CLIENT, () -> () -> MekanismUtils.getTileEntity(type, MinecraftClient.getInstance().world, buf.readBlockPos()));
    }
}