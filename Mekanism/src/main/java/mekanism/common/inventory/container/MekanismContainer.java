...
public abstract class MekanismContainer extends ScreenHandler {
    ...
    @Nonnull
    @Override
    protected Property addProperty(@Nonnull Property referenceHolder) {
    ...
    public void sendContentUpdates() {
        ...
        if (!listeners.isEmpty()) {
            ...
            if (size == 1) {
                ...
                sendChange(dirtyData.get(0).getSinglePacket((short) syncId));
            } else if (size > 1) {
                sendChange(new PacketUpdateContainerBatch((short) syncId, dirtyData));
            }
        }
    }
    ...
    @Override
    public void addListener(@Nonnull ScreenHandlerListener listener) {
        ...
        if (!alreadyHas && listener instanceof ServerPlayerEntity) {
            ...
            if (size == 1) {
                //If we only have a single element send a type specific packet to reduce overhead of
                // having to include type and count
                Mekanism.packetHandler.sendTo(dirtyData.get(0).getSinglePacket((short) syncId), (ServerPlayerEntity) listener);
            } else if (size > 1) {
                Mekanism.packetHandler.sendTo(new PacketUpdateContainerBatch((short) syncId, dirtyData), (ServerPlayerEntity) listener);
            }
        }
    }
