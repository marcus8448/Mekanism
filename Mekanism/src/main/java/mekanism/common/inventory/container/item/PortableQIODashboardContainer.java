...
public class PortableQIODashboardContainer extends QIOItemViewerContainer {
     ...
     */
    public PortableQIODashboardContainer(int id, PlayerInventory inv, PacketByteBuf buf) {
        this(id, inv, buf.readEnumConstant(Hand.class), MekanismItemContainer.getStackFromBuffer(buf, ItemPortableQIODashboard.class), true);
    }
    ...
    public PortableQIODashboardContainer recreate() {
        PortableQIODashboardContainer container = new PortableQIODashboardContainer(syncId, inv, hand, stack);
        sync(container);
        return container;
    }
    ...
    public QIOFrequency getFrequency() {
        if (!inv.player.world.isClient()) {
            ...
            FrequencyManager<QIOFrequency> manager = identity.isPublic() ? FrequencyType.QIO.getManager(null) : FrequencyType.QIO.getManager(inv.player.getUuid());
