...
public class BasicInventorySlot implements IInventorySlot {
    ...
    public int getLimit(ItemStack stack) {
        return obeyStackLimit && !stack.isEmpty() ? Math.min(limit, stack.getMaxCount()) : limit;
    }
    ...
    public CompoundTag serializeNBT() {
        ...
        if (!isEmpty()) {
            nbt.put(NBTConstants.ITEM, current.toTag(new CompoundTag()));
            if (getCount() > current.getMaxCount()) {
    ...
    public void deserializeNBT(CompoundTag nbt) {
        ...
        if (nbt.contains(NBTConstants.ITEM, NBT.TAG_COMPOUND)) {
            stack = ItemStack.fromTag(nbt.getCompound(NBTConstants.ITEM));
