...
public final class MekanismUtils {
    ...
    public static final Codec<Direction> DIRECTION_CODEC = StringIdentifiable.method_28140(Direction::values, Direction::byName);
        ...
        for (Direction side : EnumUtils.DIRECTIONS) {
            ...
            if (isBlockLoaded(world, pos) && isBlockLoaded(world, offset)) {
                ...
                if (weakPower && isDirectlyGettingPowered(world, offset)) {
                    return true;
                } else if (!weakPower && blockState.getWeakRedstonePower(world, offset, side) > 0) {
                    return true;
                }
            }
        }
    ...
    public static boolean isValidReplaceableBlock(@Nonnull BlockView world, @Nonnull BlockPos pos) {
        return World.method_24794(pos) && world.getBlockState(pos).getMaterial().isReplaceable();
    }
    ...
    public static void notifyNeighborOfChange(@Nullable World world, Direction neighborSide, BlockPos fromPos) {
        if (world != null) {
            ...
            state.neighborUpdate(world, neighbor, world.getBlockState(fromPos).getBlock(), fromPos, false);
        }
    }
    ...
    public static BlockHitResult rayTrace(PlayerEntity player) {
        return rayTrace(player, FluidHandling.NONE);
    }

    public static BlockHitResult rayTrace(PlayerEntity player, FluidHandling fluidMode) {
    ...
    public static BlockHitResult rayTrace(PlayerEntity player, double reach) {
        return rayTrace(player, reach, FluidHandling.NONE);
    }

    public static BlockHitResult rayTrace(PlayerEntity player, double reach, FluidHandling fluidMode) {
        ...
        return player.getEntityWorld().rayTrace(new RayTraceContext(headVec, endVec, ShapeType.OUTLINE, fluidMode, player));
    }
    ...
    public static boolean isChunkVibrated(ChunkPos chunk, World world) {
        return Mekanism.activeVibrators.stream().anyMatch(coord -> coord.dimension == world.getRegistryKey() && coord.getX() >> 4 == chunk.x && coord.getZ() >> 4 == chunk.z);
    }
    ...
    public static String getLastKnownUsername(UUID uuid) {
        ...
        if (ret == null && !warnedFails.contains(uuid) && EffectiveSide.get().isServer()) { // see if MC/Yggdrasil knows about it?!
            GameProfile gp = ServerLifecycleHooks.getCurrentServer().getUserCache().getByUuid(uuid);
            if (gp != null) {
                ret = gp.getName();
            }
        }
    ...
    public static void onChangedPotionEffect(LivingEntity entity, StatusEffectInstance id, boolean reapply) {
        entity.effectsChanged = true;
        if (reapply && !entity.world.isClient) {
            StatusEffect effect = id.getEffectType();
            effect.onRemoved(entity, entity.getAttributes(), id.getAmplifier());
            effect.onApplied(entity, entity.getAttributes(), id.getAmplifier());
        }
    }
