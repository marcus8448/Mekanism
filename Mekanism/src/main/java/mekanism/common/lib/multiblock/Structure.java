...
public class Structure {
    ...
    public <TILE extends BlockEntity & IMultiblockBase> void tick(TILE tile) {
        if (!didUpdate && updateTimestamp == tile.getWorld().getTime() - 1) {
            didUpdate = true;
            runUpdate(tile);
        }
        if (!isValid()) {
    ...
    private static void validate(IMultiblockBase node) {
        ...
        FormationProtocol.explore(node.getTilePos(), pos -> {
            ...
            BlockEntity tile = MekanismUtils.getTileEntity(node.getTileWorld(), pos);
