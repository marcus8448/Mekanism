...
public class ChunkManager extends PersistentState {
    ...
    private static final ChunkTicketType<ChunkPos> INITIAL_LOAD_TICKET_TYPE = ChunkTicketType.create("mekanism:initial_chunkload", Comparator.comparingLong(ChunkPos::toLong), 10);
    ...
    public static void worldLoad(ServerWorld world) {
        ...
        LOGGER.info("Loading {} chunks for dimension {}", savedData.chunks.size(), world.getRegistryKey().getValue());
        savedData.chunks.long2ObjectEntrySet().fastForEach(entry -> {
            ...
            world.getChunkManager().addTicket(INITIAL_LOAD_TICKET_TYPE, pos, TileComponentChunkLoader.TICKET_DISTANCE, pos);
        });
    }
    ...
    public static ChunkManager getInstance(ServerWorld world) {
        return world.getPersistentStateManager().getOrCreate(ChunkManager::new, SAVEDATA_KEY);
    }
}