...
public class SecurityFrequency extends Frequency {
    ...
    public void write(CompoundTag nbtTags) {
        ...
        if (!trusted.isEmpty()) {
            ...
            for (UUID uuid : trusted) {
                trustedList.add(NbtHelper.fromUuid(uuid));
            }
    ...
    protected void read(CompoundTag nbtTags) {
        ...
        if (nbtTags.contains(NBTConstants.TRUSTED, NBT.TAG_LIST)) {
            ListTag trustedList = nbtTags.getList(NBTConstants.TRUSTED, NBT.TAG_COMPOUND);
            for (int i = 0; i < trustedList.size(); i++) {
                UUID uuid = NbtHelper.toUuid(trustedList.getCompound(i));
    ...
    public void write(PacketByteBuf buffer) {
        ...
        buffer.writeEnumConstant(securityMode);
    ...
    protected void read(PacketByteBuf dataStream) {
        ...
        securityMode = dataStream.readEnumConstant(SecurityMode.class);
