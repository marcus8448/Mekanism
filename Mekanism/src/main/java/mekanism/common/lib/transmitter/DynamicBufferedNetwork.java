...
public abstract class DynamicBufferedNetwork<ACCEPTOR, NETWORK extends DynamicBufferedNetwork<ACCEPTOR, NETWORK, BUFFER, TRANSMITTER>, BUFFER,
    ...
    public final void validateSaveShares(@Nonnull TRANSMITTER triggerTransmitter) {
        ...
        if (world != null && world.getTime() != lastSaveShareWriteTime) {
            lastSaveShareWriteTime = world.getTime();
            updateSaveShares(triggerTransmitter);
        }
    }

    public void markDirty() {
        if (world != null && !world.isClient && world.getTime() != lastMarkDirtyTime) {
            lastMarkDirtyTime = world.getTime();
            chunks.forEach(chunk -> MekanismUtils.markChunkDirty(world, chunk.getCenterBlockPos()));
        }
    }
    ...
    private Range3D genPacketRange() {
        ...
        return new Range3D(minX, minZ, maxX, maxZ, world.getRegistryKey());
    }
