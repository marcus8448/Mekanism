...
public class RadiationManager {
    ...
    public void tickClient(PlayerEntity player) {
        ...
        if (clientRadiationScale != RadiationScale.NONE && player.world.getRandom().nextInt(2) == 0) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                player.world.addParticle((DefaultParticleType) MekanismParticleTypes.RADIATION.getParticleType(), x, y, z, 0, 0, 0);
            }
        }
    }
    ...
    public void createOrLoad() {
        if (dataHandler == null) {
            ...
            PersistentStateManager savedData = ServerLifecycleHooks.getCurrentServer().getOverworld().getPersistentStateManager();
    ...
    public static class RadiationDataHandler extends PersistentState {
        ...
        @Override
        public void fromTag(@Nonnull CompoundTag nbtTags) {
            if (nbtTags.contains(NBTConstants.RADIATION_LIST)) {
                ListTag list = nbtTags.getList(NBTConstants.RADIATION_LIST, NBT.TAG_COMPOUND);
