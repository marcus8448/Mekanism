...
public class EntityRobit extends PathAwareEntity implements IMekanismInventory, ISustainedInventory, ICachedRecipeHolder<ItemStackToItemStackRecipe>,
    ...
    private void collectItems() {
        List<ItemEntity> items = world.getNonSpectatingEntities(ItemEntity.class, getBoundingBox().expand(1.5, 1.5, 1.5));
        if (!items.isEmpty()) {
    ...
    public void goHome() {
        ...
        if (world.getRegistryKey() == homeLocation.dimension) {
            requestTeleport(homeLocation.getX() + 0.5, homeLocation.getY() + 0.3, homeLocation.getZ() + 0.5);
        } else {
            ...
            if (newWorld != null) {
                changeDimension(newWorld, new ITeleporter() {
                    ...
                    public Entity placeEntity(Entity entity, ServerWorld currentWorld, ServerWorld destWorld, float yaw, Function<Boolean, Entity> repositionEntity) {
                        ...
                        repositionedEntity.requestTeleport(homeLocation.getX() + 0.5, homeLocation.getY() + 0.3, homeLocation.getZ() + 0.5);
    ...
    public ScreenHandlerContext getWorldPosCallable() {
        return new ScreenHandlerContext() {
            ...
            public <T> Optional<T> run(@Nonnull BiFunction<World, BlockPos, T> worldBlockPosTBiFunction) {
                ...
                return Optional.of(worldBlockPosTBiFunction.apply(getEntityWorld(), getBlockPos()));
            }
        };
    }
}