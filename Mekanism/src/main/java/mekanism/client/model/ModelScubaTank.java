package mekanism.client.model;

import javax.annotation.Nonnull;
import mekanism.common.util.MekanismUtils;
import mekanism.common.util.MekanismUtils.ResourceType;
import net.minecraft.client.model.ModelPart;
import net.minecraft.client.render.RenderLayer;
import net.minecraft.client.render.VertexConsumer;
import net.minecraft.client.render.VertexConsumerProvider;
import net.minecraft.client.util.math.MatrixStack;
import net.minecraft.util.Identifier;

public class ModelScubaTank extends MekanismJavaModel {

    private static final Identifier TANK_TEXTURE = MekanismUtils.getResource(ResourceType.RENDER, "scuba_set.png");
    private final RenderLayer RENDER_TYPE = getLayer(TANK_TEXTURE);

    private final ModelPart tankL;
    private final ModelPart tankR;
    private final ModelPart tankDock;
    private final ModelPart capL;
    private final ModelPart capR;
    private final ModelPart tankBridge;
    private final ModelPart tankPipeLower;
    private final ModelPart tankPipeUpper;
    private final ModelPart tankBackBrace;

    public ModelScubaTank() {
        super(RenderLayer::getEntitySolid);
        textureWidth = 128;
        textureHeight = 64;

        tankL = new ModelPart(this, 23, 54);
        tankL.addCuboid(-1F, 2F, 4F, 3, 7, 3, false);
        tankL.setPivot(0F, 0F, 0F);
        tankL.setTextureSize(128, 64);
        tankL.mirror = true;
        setRotation(tankL, -0.2443461F, 0.5235988F, 0F);
        tankR = new ModelPart(this, 23, 54);
        tankR.addCuboid(-2F, 2F, 4F, 3, 7, 3, false);
        tankR.setPivot(0F, 0F, 0F);
        tankR.setTextureSize(128, 64);
        tankR.mirror = true;
        setRotation(tankR, -0.2443461F, -0.5235988F, 0F);
        tankR.mirror = false;
        tankDock = new ModelPart(this, 0, 55);
        tankDock.addCuboid(-2F, 5F, 1F, 4, 4, 5, false);
        tankDock.setPivot(0F, 0F, 0F);
        tankDock.setTextureSize(128, 64);
        tankDock.mirror = true;
        setRotation(tankDock, 0F, 0F, 0F);
        capL = new ModelPart(this, 23, 51);
        capL.addCuboid(-0.5F, 1F, 4.5F, 2, 1, 2, false);
        capL.setPivot(0F, 0F, 0F);
        capL.setTextureSize(128, 64);
        capL.mirror = true;
        setRotation(capL, -0.2443461F, 0.5235988F, 0F);
        capR = new ModelPart(this, 23, 51);
        capR.addCuboid(-1.5F, 1F, 4.5F, 2, 1, 2, false);
        capR.setPivot(0F, 0F, 0F);
        capR.setTextureSize(128, 64);
        capR.mirror = true;
        setRotation(capR, -0.2443461F, -0.5235988F, 0F);
        tankBridge = new ModelPart(this, 0, 47);
        tankBridge.addCuboid(-1F, 3F, -1.5F, 2, 5, 3, false);
        tankBridge.setPivot(0F, 0F, 0F);
        tankBridge.setTextureSize(128, 64);
        tankBridge.mirror = true;
        setRotation(tankBridge, 0.5934119F, 0F, 0F);
        tankPipeLower = new ModelPart(this, 0, 37);
        tankPipeLower.addCuboid(-0.5F, 2F, 3F, 1, 4, 1, false);
        tankPipeLower.setPivot(0F, 0F, 0F);
        tankPipeLower.setTextureSize(128, 64);
        tankPipeLower.mirror = true;
        setRotation(tankPipeLower, 0.2094395F, 0F, 0F);
        tankPipeUpper = new ModelPart(this, 4, 38);
        tankPipeUpper.addCuboid(-0.5F, 1F, 1.5F, 1, 1, 3, false);
        tankPipeUpper.setPivot(0F, 0F, 0F);
        tankPipeUpper.setTextureSize(128, 64);
        tankPipeUpper.mirror = true;
        setRotation(tankPipeUpper, 0F, 0F, 0F);
        tankBackBrace = new ModelPart(this, 0, 42);
        tankBackBrace.addCuboid(-3F, 2F, 0.5F, 6, 3, 2, false);
        tankBackBrace.setPivot(0F, 0F, 0F);
        tankBackBrace.setTextureSize(128, 64);
        tankBackBrace.mirror = true;
        setRotation(tankBackBrace, 0.2443461F, 0F, 0F);
    }

    public void render(@Nonnull MatrixStack matrix, @Nonnull VertexConsumerProvider renderer, int light, int overlayLight, boolean hasEffect) {
        render(matrix, getVertexBuilder(renderer, RENDER_TYPE, hasEffect), light, overlayLight, 1, 1, 1, 1);
    }

    @Override
    public void render(@Nonnull MatrixStack matrix, @Nonnull VertexConsumer vertexBuilder, int light, int overlayLight, float red, float green, float blue, float alpha) {
        tankL.render(matrix, vertexBuilder, light, overlayLight, red, green, blue, alpha);
        tankR.render(matrix, vertexBuilder, light, overlayLight, red, green, blue, alpha);
        tankDock.render(matrix, vertexBuilder, light, overlayLight, red, green, blue, alpha);
        capL.render(matrix, vertexBuilder, light, overlayLight, red, green, blue, alpha);
        capR.render(matrix, vertexBuilder, light, overlayLight, red, green, blue, alpha);
        tankBridge.render(matrix, vertexBuilder, light, overlayLight, red, green, blue, alpha);
        tankPipeLower.render(matrix, vertexBuilder, light, overlayLight, red, green, blue, alpha);
        tankPipeUpper.render(matrix, vertexBuilder, light, overlayLight, red, green, blue, alpha);
        tankBackBrace.render(matrix, vertexBuilder, light, overlayLight, red, green, blue, alpha);
    }
}