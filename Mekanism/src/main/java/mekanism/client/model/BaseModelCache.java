...
public class BaseModelCache {
    ...
    public static class ModelData {
        ...
        public BakedModel bake(IModelConfiguration config) {
            return bakedMap.computeIfAbsent(config, c -> model.bake(c, ModelLoader.instance(), ModelLoader.defaultTextureGetter(), SimpleModelTransform.IDENTITY, ModelOverrideList.EMPTY, rl));
        }
    ...
    public static class JSONModelData extends ModelData {
        ...
        protected void reload(ModelBakeEvent evt) {
            ...
            UnbakedModel unbaked = evt.getModelLoader().getOrLoadModel(rl);
            if (unbaked instanceof JsonUnbakedModel) {
                model = ((JsonUnbakedModel) unbaked).customData.getCustomGeometry();
            }
        }
