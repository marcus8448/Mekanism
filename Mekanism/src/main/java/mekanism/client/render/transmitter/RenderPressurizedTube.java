...
public class RenderPressurizedTube extends RenderTransmitterBase<TileEntityPressurizedTube> {
    ...
    public RenderPressurizedTube(BlockEntityRenderDispatcher renderer) {
        super(renderer);
    }

    @Override
    protected void render(TileEntityPressurizedTube tile, float partialTick, MatrixStack matrix, VertexConsumerProvider renderer, int light, int overlayLight,
          Profiler profiler) {
        ...
        if (tube.hasTransmitterNetwork()) {
            ...
            if (!network.lastChemical.isEmpty() && !network.isTankEmpty() && network.currentScale > 0) {
                ...
                renderModel(tile, matrix, renderer.getBuffer(TexturedRenderLayers.getEntityTranslucentCull()), chemical.getTint(), network.currentScale,
