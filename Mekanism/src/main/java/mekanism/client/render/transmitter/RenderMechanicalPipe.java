...
public class RenderMechanicalPipe extends RenderTransmitterBase<TileEntityMechanicalPipe> {
    ...
    public RenderMechanicalPipe(BlockEntityRenderDispatcher renderer) {
        super(renderer);
    }
    ...
    @Override
    protected void render(TileEntityMechanicalPipe tile, float partialTick, MatrixStack matrix, VertexConsumerProvider renderer, int light, int overlayLight,
          Profiler profiler) {
            ...
            if (!network.lastFluid.isEmpty() && !network.fluidTank.isEmpty() && network.currentScale > 0) {
                ...
                for (Direction side : EnumUtils.DIRECTIONS) {
                    ...
                    if (connectionType == ConnectionType.NORMAL) {
                        ...
                        connectionContents.add(side.asString() + connectionType.asString().toUpperCase(Locale.ROOT));
                    }
                    if (model != null) {
