...
public class RenderSPS extends MekanismTileEntityRenderer<TileEntitySPSCasing> {
    ...
    public RenderSPS(BlockEntityRenderDispatcher renderer) {
        super(renderer);
        CORE.setColor(Color.rgbai(255, 255, 255, 240));
    }

    @Override
    protected void render(TileEntitySPSCasing tile, float partialTick, MatrixStack matrix, VertexConsumerProvider renderer, int light, int overlayLight, Profiler profiler) {
        if (tile.isMaster && tile.getMultiblock().isFormed() && tile.getMultiblock().renderLocation != null && tile.getMultiblock().getBounds() != null) {
            ...
            Vec3d center = Vec3d.of(tile.getMultiblock().getMinPos()).add(Vec3d.of(tile.getMultiblock().getMaxPos())).add(new Vec3d(1, 1, 1)).multiply(0.5);
            ...
            if (!minecraft.isPaused() && !tile.getMultiblock().lastReceivedEnergy.isZero()) {
            ...
            if (tile.getMultiblock().lastProcessed > 0) {
                ...
                BillboardingEffectRenderer.render(CORE, tile.getPos(), matrix, renderer, tile.getWorld().getTime(), partialTick);
            }

            tile.orbitEffects.forEach(effect -> BillboardingEffectRenderer.render(effect, tile.getPos(), matrix, renderer, tile.getWorld().getTime(), partialTick));
        }
    }
    ...
    private static BoltEffect getBoltFromData(CoilData data, BlockPos pos, SPSMultiblockData multiblock, Vec3d center) {
        Vec3d start = Vec3d.ofCenter(data.coilPos.offset(data.side));
        start = start.add(Vec3d.of(data.side.getVector()).multiply(0.5));
