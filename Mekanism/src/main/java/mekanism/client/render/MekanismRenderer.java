...
public class MekanismRenderer {
    ...
    public static Sprite getSprite(Identifier spriteLocation) {
        return MinecraftClient.getInstance().getSpriteAtlas(SpriteAtlasTexture.BLOCK_ATLAS_TEX).apply(spriteLocation);
    }
    ...
    public static void renderObject(@Nullable Model3D object, @Nonnull MatrixStack matrix, VertexConsumer buffer, int argb, int light, int overlay) {
        if (object != null) {
            RenderResizableCuboid.INSTANCE.renderCube(object, matrix, buffer, argb, light, overlay);
        }
    }

    public static void renderValves(MatrixStack matrix, VertexConsumer buffer, Set<ValveData> valves, FluidRenderData data, BlockPos pos, int glow, int overlay) {
    ...
    public static int calculateGlowLight(int light, int glow) {
        ...
        return LightmapTextureManager.pack(Math.max(blockLight, glow), Math.max(skyLight, glow));
    }
    ...
    public static void onStitch(TextureStitchEvent.Post event) {
        ...
        if (!map.getId().equals(SpriteAtlasTexture.BLOCK_ATLAS_TEX)) {
            return;
        }
        for (TransmissionType type : EnumUtils.TRANSMISSION_TYPES) {
        ...
        redstoneTorch = map.getSprite(new Identifier("minecraft:block/redstone_torch"));
