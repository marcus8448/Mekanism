...
public class RenderResizableCuboid {
    ...
    protected EntityRenderDispatcher manager = MinecraftClient.getInstance().getEntityRenderManager();
    ...
    public void renderCube(Model3D cube, MatrixStack matrix, VertexConsumer buffer, int argb, int light, int overlay) {
        ...
        for (Direction face : EnumUtils.DIRECTIONS) {
            if (cube.shouldSideRender(face)) {
                ...
                Sprite sprite = cube.textures[ordinal];
                if (sprite != null) {
                    Axis u = face.getAxis() == Axis.X ? Axis.Z : Axis.X;
                    Axis v = face.getAxis() == Axis.Y ? Axis.Z : Axis.Y;
                    float other = face.getDirection() == AxisDirection.POSITIVE ? (float) getValue(size, face.getAxis()) : 0;
                    ...
                    face = face.getDirection() == AxisDirection.NEGATIVE ? face : face.getOpposite();
    ...
    private void renderPoint(Matrix4f matrix4f, Matrix3f normal, VertexConsumer buffer, Direction face, Axis u, Axis v, float other, float[] uv, float[] xyz,
        ...
        Vec3i normalForFace = face.getVector();
        ...
        norm.normalize();
        buffer.vertex(matrix4f, vertex.getX(), vertex.getY(), vertex.getZ())
              .color(red, green, blue, alpha)
              .texture(uv[U_ARRAY], uv[V_ARRAY])
              .overlay(overlay)
              .light(light)
