...
public class GuiRobitRepair extends GuiRobit<RepairRobitContainer> implements ScreenHandlerListener {
    ...
    public void init() {
        super.init();
        getMinecraft().keyboard.enableRepeatEvents(true);
        addButton(itemNameField = new TextFieldWidget(textRenderer, getGuiLeft() + 62, getGuiTop() + 24, 103, 12, new LiteralText("")));
        ...
        itemNameField.setHasBorder(false);
    ...
    private void onTextUpdate(String newText) {
        if (!newText.isEmpty()) {
            ...
            getMinecraft().player.networkHandler.sendPacket(new RenameItemC2SPacket(newText));
        }
    }
    ...
    public void removed() {
        super.removed();
        getMinecraft().keyboard.enableRepeatEvents(false);
        handler.removeListener(this);
    }
    ...
    protected void drawForegroundText(@Nonnull MatrixStack matrix, int mouseX, int mouseY) {
        ...
        if (maximumCost > 0) {
            ...
            if (flag) {
                ...
                getFont().drawWithShadow(matrix, component, width, 69.0F, k);
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        if (keyCode != GLFW.GLFW_KEY_ESCAPE && itemNameField.isActive()) {
    ...
    @Override
    protected void drawBackground(@Nonnull MatrixStack matrix, float partialTick, int mouseX, int mouseY) {
        getMinecraft().textureManager.bindTexture(ANVIL_RESOURCE);
        drawTexture(matrix, getGuiLeft(), getGuiTop(), 0, 0, getXSize(), getYSize());
        drawTexture(matrix, getGuiLeft() + 59, getGuiTop() + 20, 0, getYSize() + (handler.getSlot(0).hasStack() ? 0 : 16), 110, 16);
        if ((handler.getSlot(0).hasStack() || handler.getSlot(1).hasStack()) && !handler.getSlot(2).hasStack()) {
            drawTexture(matrix, getGuiLeft() + 99, getGuiTop() + 45, getXSize(), 0, 28, 21);
        }
    }
    ...
    @Override
    public void onPropertyUpdate(@Nonnull ScreenHandler container, int varToUpdate, int newValue) {
    }
}