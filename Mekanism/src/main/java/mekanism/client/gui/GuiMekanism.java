...
import mekanism.common.lib.LRU.LRUIterator;
...
public abstract class GuiMekanism<CONTAINER extends ScreenHandler> extends HandledScreen<CONTAINER> implements IGuiWrapper, IFancyFontRenderer {
    ...
    protected boolean isClickOutsideBounds(double mouseX, double mouseY, int guiLeftIn, int guiTopIn, int mouseButton) {
        return getWindowHovering(mouseX, mouseY) == null && super.isClickOutsideBounds(mouseX, mouseY, guiLeftIn, guiTopIn, mouseButton);
    }
    ...
    protected void drawForeground(@Nonnull MatrixStack matrix, int mouseX, int mouseY) {
        ...
        RenderSystem.translatef(-x, -y, 0);
        drawMouseoverTooltip(matrix, mouseX, mouseY);
        RenderSystem.translatef(x, y, 0);
    ...
    protected boolean isPointWithinBounds(int x, int y, int width, int height, double mouseX, double mouseY) {
        ...
        return super.isPointWithinBounds(x, y, width, height, mouseX, mouseY) &&
