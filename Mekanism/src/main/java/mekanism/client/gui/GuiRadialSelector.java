...
public class GuiRadialSelector<TYPE extends Enum<TYPE> & IRadialSelectorEnum<TYPE>> extends Screen {
    ...
    public GuiRadialSelector(Class<TYPE> enumClass, Supplier<TYPE> curSupplier, Consumer<TYPE> changeHandler) {
        super(new LiteralText("Radial Selector Screen"));
    ...
    public void render(@Nonnull MatrixStack matrix, int mouseX, int mouseY, float partialTick) {
        ...
        for (TYPE type : types) {
            ...
            // draw icon
            client.textureManager.bindTexture(type.getIcon());
            drawTexture(matrix, Math.round(x - 12), Math.round(y - 20), 24, 24, 0, 0, 18, 18, 18, 18);
            // draw label
            matrix.push();
            int width = client.textRenderer.getWidth(type.getShortText());
            matrix.translate(x, y, 0);
            matrix.scale(0.6F, 0.6F, 0.6F);
            client.textRenderer.draw(matrix, type.getShortText(), -width / 2F, 8, 0xCCFFFFFF);
            matrix.pop();
            position++;
        }
    ...
    private void drawTorus(MatrixStack matrix, float startAngle, float sizeAngle) {
        ...
        vertexBuffer.begin(GL11.GL_QUAD_STRIP, VertexFormats.POSITION);
        float draws = DRAWS * (sizeAngle / 360F);
        for (int i = 0; i <= draws; i++) {
            float angle = (float) Math.toRadians(startAngle + (i / DRAWS) * 360);
            vertexBuffer.vertex(matrix4f, (float) (OUTER * Math.cos(angle)), (float) (OUTER * Math.sin(angle)), 0).next();
            vertexBuffer.vertex(matrix4f, (float) (INNER * Math.cos(angle)), (float) (INNER * Math.sin(angle)), 0).next();
        }
        vertexBuffer.end();
