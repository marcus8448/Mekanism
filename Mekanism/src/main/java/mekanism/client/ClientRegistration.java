...
public class ClientRegistration {
    ...
    private static final Map<Identifier, CustomModelRegistryObject> customModels = new Object2ObjectOpenHashMap<>();
    ...
    public static void init(FMLClientSetupEvent event) {
        ...
        //Block render layers
        //Cutout
        ClientRegistrationUtil.setRenderLayer(RenderLayer.getCutout(), MekanismBlocks.STRUCTURAL_GLASS, MekanismBlocks.LASER_AMPLIFIER, MekanismBlocks.LASER_TRACTOR_BEAM,
        ...
        ClientRegistrationUtil.setRenderLayer(renderType -> renderType == RenderLayer.getCutout() || renderType == RenderLayer.getTranslucent(),
        ...
        for (FluidRegistryObject<?, ?, ?, ?> fluidRO : MekanismFluids.FLUIDS.getAllFluids()) {
            ClientRegistrationUtil.setRenderLayer(RenderLayer.getTranslucent(), fluidRO);
        }
        // Multi-Layer blocks (requiring both sold & translucent render layers)
        ClientRegistrationUtil.setRenderLayer(renderType -> renderType == RenderLayer.getSolid() || renderType == RenderLayer.getTranslucent(),
        ...
        ClientRegistrationUtil.setPropertyOverride(MekanismItems.ELECTRIC_BOW, Mekanism.rl("pull"),
              (stack, world, entity) -> entity != null && entity.getActiveItem() == stack ? (stack.getMaxUseTime() - entity.getItemUseTimeLeft()) / 20.0F : 0);
    ...
    public static void onModelBake(ModelBakeEvent event) {
        event.getModelRegistry().replaceAll((rl, model) -> {
            CustomModelRegistryObject obj = customModels.get(new Identifier(rl.getNamespace(), rl.getPath()));
    ...
    public static void loadComplete(FMLLoadCompleteEvent evt) {
        EntityRenderDispatcher entityRenderManager = MinecraftClient.getInstance().getEntityRenderManager();
        //Add our own custom armor layer to the various player renderers
        for (Entry<String, PlayerEntityRenderer> entry : entityRenderManager.getSkinMap().entrySet()) {
    ...
    private static <T extends LivingEntity, M extends BipedEntityModel<T>, A extends BipedEntityModel<T>> void addCustomArmorLayer(LivingEntityRenderer<T, M> renderer) {
        for (FeatureRenderer<T, M> layerRenderer : new ArrayList<>(renderer.features)) {
            //Only allow an exact match, so we don't add to modded entities that only have a modded extended armor layer
            if (layerRenderer.getClass() == ArmorFeatureRenderer.class) {
                ArmorFeatureRenderer<T, M, A> bipedArmorLayer = (ArmorFeatureRenderer<T, M, A>) layerRenderer;
                renderer.addFeature(new MekanismArmorLayer<>(renderer, bipedArmorLayer.leggingsModel, bipedArmorLayer.bodyModel));
                break;
            }
        }
    }
