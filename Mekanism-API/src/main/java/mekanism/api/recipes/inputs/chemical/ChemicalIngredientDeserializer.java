...
public class ChemicalIngredientDeserializer<CHEMICAL extends Chemical<CHEMICAL>, STACK extends ChemicalStack<CHEMICAL>,
    ...
    private final Function<PacketByteBuf, STACK> fromPacket;
    private final Function<Identifier, CHEMICAL> fromRegistry;
    ...
    private ChemicalIngredientDeserializer(String name, ChemicalIngredientInfo<CHEMICAL, STACK> info, ChemicalTags<CHEMICAL> tags, Function<PacketByteBuf, STACK> fromPacket,
          Function<Identifier, CHEMICAL> fromRegistry, Function<STACK, INGREDIENT> stackToIngredient, TagIngredientCreator<CHEMICAL, STACK, INGREDIENT> tagToIngredient,
    ...
    public final INGREDIENT read(PacketByteBuf buffer) {
        ...
        if (type == IngredientType.SINGLE) {
            ...
            return tagToIngredient.create(ChemicalTags.chemicalTag(buffer.readIdentifier(), tags), buffer.readVarLong());
        }
    ...
    public final STACK deserializeStack(@Nonnull JsonObject json) {
        ...
        Identifier resourceLocation = new Identifier(JsonHelper.getString(json, info.getSerializationKey()));
