...
public class ItemMekanismAxe extends AxeItem implements IHasRepairType, IAttributeRefresher {
    ...
    public float getMiningSpeedMultiplier(@Nonnull ItemStack stack, BlockState state) {
        if (field_23139.contains(state.getMaterial()) || getToolTypes(stack).stream().anyMatch(state::isToolEffective) || effectiveBlocks.contains(state.getBlock())) {
    ...
    public boolean isEffectiveOn(BlockState state) {
        return state.getHarvestTool() == ToolType.AXE ? getHarvestLevel() >= state.getHarvestLevel() : super.isEffectiveOn(state);
    }
    ...
    public void addToBuilder(ImmutableMultimap.Builder<EntityAttribute, EntityAttributeModifier> builder) {
        ...
        builder.put(EntityAttributes.GENERIC_ATTACK_SPEED, new EntityAttributeModifier(ATTACK_SPEED_MODIFIER_ID, "Tool modifier", material.getAxeAtkSpeed(), Operation.ADDITION));
    }
}